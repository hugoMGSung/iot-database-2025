...
def showDatas():
    ...
    conn = oci.connect(f'{username}/{password}@{host}:{port}/XE')
    ...
    # data = cursor.fetchall()    # 쿼리실행 데이터 전부 가져옴

    ### 가져온 데이터 트리뷰에 추가
    # print(data)
    dataView.delete(*dataView.get_children())       # 최초, 중간에 showDatas() 호출시마다 트리뷰 클리어
    for i, (std_id, std_name, std_mobile, std_regyear) in enumerate(cursor, start=1): # enumerate - 인덱스와 아이템 동시접근
...
def addData():
    ...
    try:
        ...
        # 쿼리 작성
        query = 'insert into students values (SEQ_STUDENT.nextval, :v_std_name, :v_std_mobile, :v_std_regyear)'
...
def modData():
    ...
    try:
        conn.begin()    # begin transaction. 트랜잭션 시작
        # 쿼리 작성
        query = 'update students set std_name=:v_std_name, std_mobile=:v_std_mobile, std_regyear=:v_std_regyear where std_id=:v_std_id'
...
def delData():
    ...
    try:
        conn.begin()    # begin transaction. 트랜잭션 시작
        # 쿼리 작성
        query = 'delete from students where std_id=:v_std_id'
...
ent1.config(state='readonly', foreground='blue', disabledforeground='blue')  # 값을 입력못하게 방지
